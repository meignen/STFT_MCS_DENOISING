function [SNR_modes] = test_reconstruct_three_case_modes(cas,method,sig,window,SNR,nbreal)
 %INPUT
 ...
 downsamp  = [16 32 64];
 SNR_modes = cell(1,length(downsamp));
 ...
 for nb = 1:nbreal
  ...
  if (method == 1)
   for p = 1:length(downsamp),
    ...
    SNR_int = zeros(nr,downsamp(p));
    while shift < downsamp(p) 
     shift
     [tfr,norm2h] = tfrstft_three_case_down(sn,Nfft,cas,h,Lh,downsamp(p),shift); 
     ...
     for j=1:nr,
      ...
      modes(:,j) = itfrstft_three_case_down(tfr_int,cas,N,h,shift);
      SNR_int(j,shift+1) = snr(Sacc(:,j),modes(:,j)-Sacc(:,j));
     end
     shift = shift + 1; 
    end
    if nb == 1
...
for p = 1:length(downsamp)
 SNR_modes{p} = SNR_modes{p}/nbreal;
end  
end
